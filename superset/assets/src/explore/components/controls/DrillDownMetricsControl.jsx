...
import DrillDownFilterOption from '../DrillDownFilterOption';
...
export default class DrillDownMetricsControl extends React.Component {
    ...
    this.props.onChange(opts.map((option) => {
      if (option.saved_metric_name) {
        ...
        return new AdhocFilter({
          ...
          subject: this.props.datasource.type === 'druid' ?
            option.label :
            new DrillDownMetric(option).translateToSql(),
          operator: OPERATORS['>'],
          comparator: 0,
          clause: CLAUSES.HAVING,
        });
  ...
  optionsForSelect(props) {
    const options = [
      ...props.columns,
      ...[...(props.formData && props.formData.metrics || []), props.formData && props.formData.metric].map(metric => (
        metric && (
          typeof metric === 'string' ?
            { saved_metric_name: metric } :
            new DrillDownMetric(metric)
        )
      )),
    ].filter(option => option);
    ...
    return options.reduce((results, option) => {
      ...
      } else if (option instanceof DrillDownMetric) {
...
DrillDownMetricsControl.propTypes = propTypes;
DrillDownMetricsControl.defaultProps = defaultProps;
