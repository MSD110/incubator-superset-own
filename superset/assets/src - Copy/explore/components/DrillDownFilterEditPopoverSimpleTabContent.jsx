...
const propTypes = {
  drillDownFilter: PropTypes.instanceOf(AdhocFilter).isRequired,
...
export default class DrillDownFilterEditPopoverSimpleTabContent extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.onDrillDownChange = this.onDrillDownChange.bind(this);
  ...
  onDrillDownChange(option) {
    console.log('onDrillDownChange 1', option, this.props.drillDownFilter)
    let subject;
    let clause;
    // infer the new clause based on what subject was selected.
    if (option && option.column_name) {
    ...
    this.props.onDrillDownChange(
      this.props.drillDownFilter.duplicateWith({
        subject,
        clause,
        expressionType: EXPRESSION_TYPES.SIMPLE,
      }),
  ...
  render() {
    const { drillDownFilter, adhocFilter, options, datasource } = this.props;
    ...
    let drillDownSelectProps = {...subjectSelectProps, onChange: this.onDrillDownChange, value: drillDownFilter.subject ? { value: drillDownFilter.subject } : undefined,};
    ...
    return (
      <span>
        <div>For</div>
        <FormGroup className="adhoc-filter-simple-column-dropdown" style={{marginTop: 5}}>
        ...
        <div style={{marginTop: 15}}>Drill Down by</div>
        <FormGroup className="adhoc-filter-simple-column-dropdown" style={{marginTop: 5}}>
          <OnPasteSelect {...this.selectProps} {...drillDownSelectProps} />
        </FormGroup>
      </span>
    );
  }
}
DrillDownFilterEditPopoverSimpleTabContent.propTypes = propTypes;
DrillDownFilterEditPopoverSimpleTabContent.defaultProps = defaultProps;
